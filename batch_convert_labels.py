...
def yolo_to_corners_normalized(label_path, img_width, img_height):
...
def save_new_labels(new_labels, output_path):
    with open(output_path, 'w') as f:
        for label in new_labels:
            formatted_label = [label[0]] + [f"{coord:.6f}" for coord in label[1:]]
            f.write(' '.join(map(str, formatted_label)) + '\n')
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith('.txt'):
                ...
                new_labels = yolo_to_corners_normalized(input_path, img_width, img_height)
                save_new_labels(new_labels, output_path)


input_dir = 'labels_input'
output_dir = 'labels'
img_width = 1920
img_height = 1080
